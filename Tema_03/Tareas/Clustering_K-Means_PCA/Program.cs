...
for (int r = 2; r <= 4; r++)
{
    ...
    var pcaPipeline = preprocessingPipeline.Append(mlContext.Transforms.ProjectToPrincipalComponents(outputColumnName: "PcaFeatures",inputColumnName: "Features",rank: r));

    var clusteringPipeline = pcaPipeline.Append(mlContext.Clustering.Trainers.KMeans(featureColumnName: "PcaFeatures",numberOfClusters: k));
